...
knn(irisTrain[,1:2], c(6.2, 2.8), irisTrain$Species, k = 5, prob = TRUE)
knn(irisTrain[,1:2], c(6.2, 2.8), irisTrain$Species, k = 15, prob = TRUE)

knn(irisTrain[,1:2], c(6.1, 2.5), irisTrain$Species, k = 5, prob = TRUE)
knn(irisTrain[,1:2], c(6.1, 2.5), irisTrain$Species, k = 10, prob = TRUE)
knn(irisTrain[,1:2], c(6.1, 2.5), irisTrain$Species, k = 15, prob = TRUE)

knn(irisTrain[,1:2], c(6.2, 4.9), irisTrain$Species, k = 5, prob = TRUE)
knn(irisTrain[,1:2], c(6.2, 4.9), irisTrain$Species, k = 10, prob = TRUE)
knn(irisTrain[,1:2], c(6.2, 4.9), irisTrain$Species, k = 15, prob = TRUE)
...
```{r}
x = knn.cv(irisVersVirg[, 1:2], irisVersVirg$Species, k = 10)
confusionMatrix(table(tibble(x,irisVersVirg$Species)))
```
...
dfZTrain = tibble(Zincome = scale(dfTrain$income), ZCreditRating = scale(dfTrain$CreditRating), Qualify = dfTrain$Qualify)
...
credit = read_csv("/Users/andrewyule/Dropbox/Personal/SMU MSDS/03 - Doing Data Science/MSDS_6306_Doing-Data-Science/Unit 6/Credit Default.csv")
...
#Standardized
classifications = knn(train[,c(15,16)],test[,c(15,16)],train$`default payment next month` = TRUE, k = 5)
confusionMatrix(table(classifications,test$`default payment next month`))
...
classifications = knn.cv(credit[,c(15,16)],credit$`default payment next month`,prob = TRUE, k = 50)
confusionMatrix(table(classifications,credit$`default payment next month`))

classifications = knn.cv(credit[,c(15,16)],credit$`default payment next month`,prob = TRUE, k = 90)
confusionMatrix(table(classifications,credit$`default payment next month`))
...
# Archeology
```{r}
pottery = read_csv("/Users/andrewyule/Dropbox/Personal/SMU MSDS/03 - Doing Data Science/MSDS_6306_Doing-Data-Science/Unit 6/potterydata.csv")
pottery
...
confusionMatrix(table(factor(rep("L", dim(pottery)[1]), levels = c("A", "C", "I", "L")), pottery$Site))
