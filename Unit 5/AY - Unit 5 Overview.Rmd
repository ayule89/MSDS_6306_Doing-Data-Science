...
Acu = read_csv("/Users/andrewyule/Dropbox/Personal/SMU MSDS/03 - Doing Data Science/MSDS_6306_Doing-Data-Science/Unit 5/Acuspike Customer Data.csv") # read in company data

lookup = tibble(abb = state.abb, State = state.name) #makes a data frame with State name and abbreviation. 

colnames(Acu)[2] = "abb" # Change Column Name

Acu2 = inner_join(Acu, lookup, by = "abb")
...
map.df = left_join(states, AcuMapData2, by="region")
...
df = tibble(quotes = c(string2,string4,string5,string6))
df$quotes
...
str_view_all(df$quotes, "0*") #Greedy
str_view(df$quotes, "0+") #Greedy
str_view_all(df$quotes, "[sn]+") #Greedy
...
sum(str_detect(c("C", "A", "P", "A", "A"), "A"))
sum(str_count(c("C", "A", "P", "A", "A"), "A"))
```
...
matches
matches_all = str_extract_all(has_color,color_expression, simplify = T)
matches_all
...
roster$Major = str_replace(roster$Major, "(\\?|)", "<NA>")
...
NYTIMES_KEY = "fZbb9qlfRkSF5aU6i7bdcEdAWG1kX3WP" #Your Key Here … get from NTY API website
